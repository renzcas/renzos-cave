...
//  Includes: Fog, Mist, Dust, Wraith, Crystals, Heart Core
//  + 3D Beast System (Wolf, Bear, Trog in Room 1)
//  + Beast Sounds (idle, snarl, attack)
// ------------------------------------------------------------
...
import { spawnBeast } from "./cave/beasts/beast-3d.js";
import { BeastTextures } from "./cave/beasts/beast-registry.js";
import { updateBeastAI } from "./cave/beasts/beast-ai.js";
import { attachBeastSounds } from "./cave/beasts/beast-sounds.js";
...
// ------------------------------------------------------------
//  BEASTS — ROOM 1 ONLY (Wolf, Bear, Trog)
// ------------------------------------------------------------

const beasts = [];

function spawnRoom1Beasts() {
  const wolf = spawnBeast(scene, BeastTextures.beast_wolf, 1, 0.2, -2);
  attachBeastSounds(wolf, listener, "wolf");
  beasts.push(wolf);

  const bear = spawnBeast(scene, BeastTextures.beast_bear, -2, 0.2, -3);
  attachBeastSounds(bear, listener, "bear");
  beasts.push(bear);

  const trog = spawnBeast(scene, BeastTextures.trog, 0, 0.2, 1);
  attachBeastSounds(trog, listener, "trog");
  beasts.push(trog);
}
